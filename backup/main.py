...
def main():
    ...
    # BGMの読み込み
    bgm_file = Path("bgm/current_pattern.wav")
    bgm_sound = None
    if bgm_file.exists():
        try:
            bgm_sound = pygame.mixer.Sound(str(bgm_file))
        except Exception as e:
            print(f"BGM読み込みエラー: {e}")
    ...
    while True:
        ...
        # BGMの再生制御（試合中のみ）
        if main_game.state == GameState.GAME and bgm_sound:
            if not bgm_channel or not bgm_channel.get_busy():
                bgm_channel = bgm_sound.play(-1)  # -1でループ再生
        elif bgm_channel:
            bgm_channel.stop()
            bgm_channel = None
        ...
        if main_game.state == GameState.TITLE:
            ...
            # タイトル画面の描画（HUDなし）
            main_game.draw(screen, draw_hud=False)
        else:
            # ゲームプレイ時は通常通りHUDを含めて描画
            main_game.draw(screen, draw_hud=True)
