...
from game.sound_effects import play_sound  # 効果音モジュールをインポート
...
class Player:
    ...
    def update(self, keys, arena, projectiles, enemy=None):
        """プレイヤーの状態を更新"""
        # 相手の方向を計算
        dx, dy = 0, 0
        if enemy:
        ...
        # シールド更新
        if keys["shield"] and self.shield_cooldown <= 0 and not self.is_shield_active:
        ...
        if self.is_shield_active:
            ...
            # シールドエフェクトを更新
            if hasattr(self, 'shield_effect') and self.shield_effect:
                self.shield_effect.update()
            
            if self.shield_duration <= 0 or not keys["shield"]:
                self.is_shield_active = False
        ...
        # 武器処理
        if enemy:  # 敵がいる場合のみ武器処理を行う
            self.handle_weapons(keys, projectiles, enemy)
        
        # ヒートゲージ処理
        if self.heat > 0:
            # 通常時のヒート減少
            decrease_rate = HEAT_DECREASE_RATE
            
            # アリーナの境界付近では冷却が速くなる
            if arena.is_near_border(self.x, self.y):
                decrease_rate *= 3
            
            self.heat = max(0, self.heat - decrease_rate)
        
        # オーバーヒート状態の処理
        if self.heat >= MAX_HEAT:
            if not self.is_overheated:
                self.is_overheated = True
                self.overheated_duration = 180  # 3秒間（60fps想定）
        
        if self.is_overheated:
            self.overheated_duration -= 1
            if self.overheated_duration <= 0:
                self.is_overheated = False
        
        # ハイパーゲージ処理
        if self.hyper > 0 and not self.is_hyper_active:
            # アリーナの境界付近ではゲージ減少が速くなる
            if arena.is_near_border(self.x, self.y):
                self.hyper = max(0, self.hyper - HYPER_DECREASE_RATE_AT_BORDER)
        
        # ハイパーモード処理
        if self.is_hyper_active:
            self.hyper_duration -= 1
            self.hyper = max(0, self.hyper - HYPER_CONSUMPTION_RATE)
            
            # ゲージが0になるか時間切れで終了
            if self.hyper <= 0 or self.hyper_duration <= 0:
                self.is_hyper_active = False
                self.hyper = 0  # 完全に使い切る
        
        # ハイパーモード発動条件: ハイパーキーを押す + ゲージが50%以上 + 発動していない
        if keys["hyper"] and self.hyper >= HYPER_ACTIVATION_COST and not self.is_hyper_active:
            self.is_hyper_active = True
            self.hyper_duration = HYPER_DURATION
            self.hyper -= HYPER_ACTIVATION_COST  # 発動コスト
        ...
        if self.weapon_b_burst_active:
            ...
            if self.weapon_b_burst_timer <= 0 and self.weapon_b_burst_count < self.weapon_b_burst_total:
                ...
                if self.weapon_b_burst_count < self.weapon_b_burst_total:
                    ...
                    self.is_special_spread_active = False
    ...
    def handle_weapons(self, keys, projectiles, enemy=None):
        """武器処理"""
        # シールド中は攻撃できない
        if self.is_shield_active:
            return
        ...
        if keys["weapon_a"] and self.shoot_cooldown <= 0:
            self.shoot(angle, projectiles, "beam")
        ...
        if keys["special"] and keys["weapon_b"] and self.shoot_cooldown <= 0:
            ...
            if self.hyper >= 100:
                ...
                # ゲージ不足の場合は通常の攻撃
                weapon = self.weapons["weapon_b"]
                projectile = self.create_projectile(weapon, enemy)
                if projectile and self.game:
                    self.game.add_projectile(projectile)
                self.shoot_cooldown = weapon.cooldown
            
        # ハイパーモード発動（ハイパーキーを押してゲージが十分ある場合）
        if keys["hyper"] and not self.is_hyper_active and self.hyper >= HYPER_ACTIVATION_COST:
            self.activate_hyper()
    ...
    def create_projectile_with_angle(self, weapon, angle):
        ...
        # ハイパーゲージも上昇（7から15に増加）
        self.hyper = min(MAX_HYPER, self.hyper + 15)
    ...
    def activate_hyper(self):
        ...
        if self.hyper >= HYPER_ACTIVATION_COST:
            self.is_hyper_active = True
            self.has_fired_hyper_laser = False  # ハイパーレーザーを発射していない状態にリセット
            self.hyper -= HYPER_ACTIVATION_COST  # 初期コストのみ消費
            ...
            if self.game:
                ...
                # 効果音を再生
                if "hyper" in self.game.sounds:
                    self.game.sounds["hyper"].play()
    ...
    def take_damage(self, amount):
        ...
        if not self.is_shield_active:
            ...
            self.hyper = min(MAX_HYPER, self.hyper + amount / 5)
            # ヒット効果音
