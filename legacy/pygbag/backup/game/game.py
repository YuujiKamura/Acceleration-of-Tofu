...
from game.sound_effects import play_sound, initialize_sounds  # 効果音モジュールをインポート
...
class Game:
    ...
    def handle_keydown(self, key):
        ...
        if self.state == GameState.TITLE:
            ...
            elif key == pygame.K_DOWN and self.selected_option < 2:
                self.selected_option += 1
                play_menu_sound = True
            ...
            # 決定キー
                ...
                menu_select = True
                if self.selected_option == 0:  # ゲーム開始
                    self.state = GameState.GAME
                    self.init_game()
                elif self.selected_option == 1:  # トレーニングモード
                    self.state = GameState.TRAINING
                    self.init_training()
                elif self.selected_option == 2:  # 自動テストモード
    ...
    def draw(self, screen, draw_hud=True):
        """描画処理 - ズーム機能付き"""
        if self.state == GameState.GAME or self.state == GameState.TRAINING or self.state == GameState.AUTO_TEST:
            # プレイヤー間の距離を計算
            dx = self.player1.x - self.player2.x
            ...
            # 自動テストモードの情報表示
            if self.state == GameState.AUTO_TEST and draw_hud:
    ...
    def check_projectile_collisions(self):
        ...
        for proj in self.projectiles[:]:
            ...
            # アリーナ外に弾が出たかチェック
            if not self.arena.is_inside(proj.x, proj.y):
                if proj in self.projectiles:
                    self.projectiles.remove(proj)
                continue
            
            # Player1がPlayer2の攻撃に当たった場合
            if proj.owner == self.player2 and self.check_player_hit(self.player1, proj):
                if self.player1.is_shield_active:
                    # シールド効果音は必要に応じて追加
                    pass
                else:
                    # ダメージ計算
                ...
                # 衝突した弾を削除（メレー攻撃以外）
                if proj.weapon_type != WEAPON_TYPES["MELEE"]:
                    self.projectiles.remove(proj)
                continue
            
            # Player2がPlayer1の攻撃に当たった場合
            if proj.owner == self.player1 and self.check_player_hit(self.player2, proj):
                if self.player2.is_shield_active:
                    # シールド効果音は必要に応じて追加
                    pass
                else:
                    # ダメージ計算
